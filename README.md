...
Use `go get` to compile the binary: `go get -v github.com/riking/go-redirect-agent`

Set AGENT\_HOST to the secure server path hosting the go link redirection service.
...
sudo --preserve-env=AGENT_HOST,AGENT_LISTEN,GOPATH,HOME bash -e
# ensure compile succeeded
test -f "$(go env GOBIN)/go-redirect-agent"
install "$(go env GOBIN)/go-redirect-agent" /usr/local/bin/go-redirect-agent
